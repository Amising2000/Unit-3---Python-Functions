...
def create_card(name, level=1, active=False):
    return {"Name": name, "level": level, "Active": active}


# Test Question 2
print(
    create_card("Fireball")
)  # Should return: {'name': 'Fireball', 'level': 1, 'active': False}
print(
    create_card("Shield", level=3, active=True)
)  # Should return: {'name': 'Shield', 'level': 3, 'active': True}
...
# Test Question 4
print(
    build_profile("gamer42", score=850, rank="Gold")
)  # Should return: {'username': 'gamer42', 'score': 850, 'rank': 'Gold'}
